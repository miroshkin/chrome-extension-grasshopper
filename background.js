...
chrome.runtime.onInstalled.addListener(() => {
    chrome.storage.sync.get(['jiraUrl1', 'jiraUrl2'], function(result) {
        if (!result.jiraUrl1) {
            chrome.storage.sync.set({ jiraUrl1: 'https://chromeextension.atlassian.net/browse/' });
        }
        if (!result.jiraUrl2) {
            chrome.storage.sync.set({ jiraUrl2: 'https://chromeextension.atlassian.net/browse/' });
        }
    });
});
